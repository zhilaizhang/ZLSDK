...
{
    isStop = 0;
    connSocket_2CB(env,jcb,port,jcb2);
}
        ...
        if (n>0)
        {
            ...
            if(lastRtpPosition+n < 4096)
            {
                ...
                for (int i = 5; i < lastRtpPosition-2; ++i) {
                    if(*(lastRtpBuff+i) == 0x80 && ( *(lastRtpBuff +i + 1)== 0x60 || *(lastRtpBuff +i + 1)== 224 ))
                    {
                        mark = i;
                        break;
                    }
...
{
    connSocket_2CB(env,gObj,port,NULL);
}
...
//                    writeData(lastRtpBuff,mark);
//                    writeData("\n\n",2);
                        LOGE("disposeData:begin:%d,mark:%d,buffPosition:%d,state:%d,%d", buff[ begin], buff[ begin + 1],
                             buff[ begin + 2],buff[ begin + 3],buff[ begin + 4]);
//                        writeData(buff + begin,mark-begin);
//                        writeData("\n\n",2);
                        begin += 5;
                        LOGE("type======:%d",buff[begin -1]);
                        if(buff[begin -1 ] == 1)
                        {
                            LOGE("jcb cb");
                            disposeData(buff + begin , 0, mark - begin, env, gObj);
                        } else if(jcb2 != NULL )
                        {
                            LOGE("jcb cb2");
                            disposeData(buff + begin, 0, mark - begin, env, jcb2);
                        }
                        begin = mark;
...
void disposeData(char* data,int offset,int length,JNIEnv * env, jobject gObj )
{
    ...
    if(showLog && length != 1400)
        LOGI("type:%d,step:%d,dataLength:%d,serialNum:%d,lostNum:%d",
             type,step,length,serialNum,lostAllPckNum);
    //没有丢序列号
    if(type == 28 )//分片的
    {
        if( step == 2 )//开始
        ...
        }else if(step == 1)//一帧的结束
        {
            ...
            memcpy(cacheBuff + position ,data + 14,length - 14 );
            position += length - 14 ;
            int a = (receiveCutNum-1)*1386+5+length-14;
            if(position != a )
            {
                LOGE("TYPE:%d,lostCutNum:%d,receiveCutNum:%d,length:%d,needLength:%d",
                     *(cacheBuff+4),lostCutNum,receiveCutNum,position,a);
            }
            if(showLog)
